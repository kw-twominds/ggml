                ...
                case GGML_OP_CONV_2D:
                    {
                        ...
                        const int ne00 = node->src0->ne[0]; // W
                        const int ne01 = node->src0->ne[1]; // H
                        const int ne02 = node->src0->ne[2]; // C
                        const int ne03 = node->src0->ne[3]; // N

                        const int ne10 = node->src1->ne[0]; // W
                        const int ne11 = node->src1->ne[1]; // H
                        const int ne12 = node->src1->ne[2]; // C

                        const int nk = ne00*ne01;

                        size_t cur = 0;

                        if (node->src0->type == GGML_TYPE_F16 &&
                            node->src1->type == GGML_TYPE_F32) {
                            cur = sizeof(ggml_fp16_t)*(nk*ne02*ne03 + ne10*ne11*ne12);
                        } else if (node->src0->type == GGML_TYPE_F32 &&
                                   node->src1->type == GGML_TYPE_F32) {
                            cur = sizeof(float)*(nk*ne02*ne03 + ne10*ne11*ne12);
                        } else {
                            GGML_ASSERT(false);
                        }
